...
- Language: **key visuals (ŹOOĻ / DiŹ wordmarks, hero, tour branding) are never in Chinese** — they stay Latin/stylized. Everything outside the key visuals (body copy, UI, section content) **defaults to Chinese and is switchable to Japanese / English**.
...
2. **乐队简介 / Band Profile** — intro to **ŹOOĻ, a five-member band** (copy/member details supplied by creator later; placeholder/fictional content OK for now).
...
Terminology: **TSUKUMO99** = series/label; **ŹOOĻ** = the band (primary wordmark); **DiŹ** = this tour; tagline **"Once and for all, dice away."**
...
- Names/relationship: "**TSUKUMO99 presents ŹOOĻ**" — ŹOOĻ leads as wordmark, TSUKUMO99 is the series/label sub-mark.
...
- The main key art (`assets/hero.*`) is **fan art of ŹOOĻ's five members** — it depicts the band itself, not a decorative hero image, and should be treated as the band's "real" group visual.

## Evidence on Hand

- `DESIGN_BRIEF.md` — the creator's own brief (band relationship, tour list, flow, ticket requirements).
- Working incumbent site: `index.html` + `script.js` (client-rendered) + `styles.css`; `assets/hero.*` = fan art of ŹOOĻ's five members, dice favicon, cheer/荧光棒 widget, working ticket PNG export.
